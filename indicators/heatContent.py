...
## \brief compute heat content from a given depth to to the surface
...
def exitMessage(message='', errorCode=1):
    if message!='':
        print message
    print 'Usage: heatContent.py -o outfile [-theta thetvarname=theta0] [-rho rhovarname=rhopoto] [-of outputformat] [-co options] -frho rhopotofile -ftheta thetaofile'
    sys.exit(errorCode)
...
def computeHeatPotential(profileTheta, profileRho, zlevels, threshold=700, nodata=1e+20):
    if (profileTheta.max() >= nodata) or (profileRho.max() >= nodata):
        return -1
    ...
    C = 3993
    zprofile=numpy.array(list(takewhile(lambda e:e>=threshold, zlevels) ))
    if len(zprofile) == 0:
        return 0
...
if __name__=="__main__":
    ...
    doHeatContent(theta_file, thetao_var, rho_file, rho_var, nodata, thresholdDepth, outfile, outformat, options)
