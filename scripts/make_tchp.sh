...
do
    ...
    echo 'calling tchp.py'
    ${srcDir}/indicators/tchp.py -o ${outDir}/${timeFrame}_tchp.nc -of gtiff -co "compress=lzw" -v thetao -frho ${rhofile}  -ftheta ${infile}
    if [[ $? -ne 0 ]]; then
	echo "processing tchp.py failed; Exit."
	exit
    fi
    ${binDir}/max_yearly.py -o ${outDir}/${timeFrame}_max_tchp.tif ${outDir}/${timeFrame}_tchp.tif
done